...
function formatSnapshotList(list, limit) {
  ...
  if (hasMore) {
    return (
      buildList(limitedList.slice(0, -1)) +
...
function snapshotFramework(files, config, emitter, loggerFactory) {
  ...
  snapshotServer.configureLogger(logger);
  ...
  if (snapshotConfig.update) {
    files.unshift(
      filePattern(path.join(__dirname, "snapshot-state-update.js"))
    );
  }
  ...
  emitter.on("browser_complete", (clientInfo, data) => {
    ...
    snapshotServer.stop();
    data = data || {};
    data.snapshot = snapshotStore.getSnapshots();
    if (lastResult.disconnected) {
      logger.warn("Browser disconnected. Snapshot might not be valid!");
    }
      ...
      if (
        ...
        if (prunedSnapshots.pruned.length > 0) {
          if (snapshotConfig.prune) {
            ...
            prune.pruneFiles(
              snapshotConfig.pathResolver,
              config.basePath,
              prunedSnapshots.prunedFiles
            );
            dirty = true;
          } else {
            logger.warn(
              formatUnusedSnapshotsWarning(
                ...
                snapshotConfig.limitUnusedSnapshotsInWarning
              )
            );
          }
        }
      ...
      if (dirty) {
        Object.keys(rootSuite.children).forEach(suiteName => {
          ...
          if (suite.dirty) {
            ...
            const snapshotPath = snapshotConfig.pathResolver(
              config.basePath,
              suiteName
            );
...
function snapshotPreprocessor(basePath, loggerFactory) {
  ...
  return function(content, file, done) {
    ...
    snapshotStore.addSuite(root.name, root.suite);
    done(
      iifeWrapper(
        'window.__snapshot__.addSuite("' +
          root.name +
          '",' +
          JSON.stringify(root.suite) +
          ");"
      )
    );
  };
